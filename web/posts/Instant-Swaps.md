...
The matching engine chooses the bid that gives you the best amount (the largest amount of the other token you want to swap into) for the current token that you have.

Basically, let's say you are swapping from BTC to USDC: the engine will pick the bid that gives you the most BTC for the USDC you currently own as the winning bid.

Different liquidity providers use different algorithms and do not see each other's bids. They submit their bids based on their assessment of the market and the risk they are willing to take. If they perceive a token as volatile, they might quote a higher price or abstain from bidding.
